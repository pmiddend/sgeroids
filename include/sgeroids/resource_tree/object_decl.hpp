...
\tparam T A copyable, assignable resource type such as a <code>shared_ptr</code> to a resource (a texture/sound/etc.)
...
In sgeroids, we load all resources at once, since we're expecting to fit all
into system and video memory. sgeroids::resource_tree::object helps us with
that.
...
<ol>
	...
	<li>
		...
		change the code</em>. The sgeroids::resource_tree::object::get
...
sgeroids::resource_tree::element objects. Continue to the documentation for
...
public:
	...
	*/
	typedef typename
	resource_tree::path_to_resource_function<value_type>::type
